...
export default function ExamDetailsPage() {
  ...
  const fetchExamDetails = async () => {
    try {
      setLoading(true)
      // MODIFIED: Don't fetch questions - only metadata
              ...
              <div className="pt-4">
                {exam.isFree || exam.isPurchased ? (
                  ...
                  <Button 
                    size="lg" 
                    className="w-full md:w-auto"
                    onClick={() => toast.info('Payment integration coming in Phase 6')}
                  >
                    <Lock className="mr-2 h-5 w-5" />
                    Purchase for ₹{exam.price / 100}
                  </Button>
                )}
              </div>
            </div>
