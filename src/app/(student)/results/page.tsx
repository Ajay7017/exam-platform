...
export default async function ResultsPage() {
  ...
  // ✅ GET GLOBAL RANK (best rank across all exams)
  const globalRank = await prisma.leaderboardEntry.findFirst({
    where: { userId: session.user.id },
    orderBy: { rank: 'asc' }, // Get best rank
    select: { rank: true }
  })
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {globalRank?.rank ? `#${globalRank.rank}` : 'N/A'}
                </p>
              </div>
