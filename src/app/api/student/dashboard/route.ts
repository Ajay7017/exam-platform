...
export async function GET(request: NextRequest) {
  try {
    ...
    // 4. Get user's best rank and total participants
    const userLeaderboard = await prisma.leaderboardEntry.findMany({
      where: { userId },
      orderBy: { rank: 'asc' },
      take: 1
    })

    const bestRank = userLeaderboard[0]?.rank || null

    // Count total unique participants across all exams
